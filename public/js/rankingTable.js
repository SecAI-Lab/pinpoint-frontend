export function renderRankingTable(rankTableWrap, detailWrap, rankings, currentTarget, onSelectCandidate, detailFuncs) {
  ...
  let html = `<table class="rank"><thead><tr>
    <th>Rank</th><th>Score</th><th>Stage</th><th>Candidate</th><th>Label</th><th>Tokens</th><th>Selected window</th>
  </tr></thead><tbody>`;

  const bestWindowCell = (r) => {
    if (r.best_stage === 'stage2' && r.best_window_bb) {
      return `[${r.best_window_bb[0]}:${r.best_window_bb[1]}] BB`;
    }
    if (r.target_range) {
      return `[${r.target_range[0]}:${r.target_range[1]}] tok`;
    }
    return 'n/a';
  };
  ...
  const rowHtml = (r) => {
    ...
    const tip = withDetail && !withDetail.has(r.func_name)
      ? ' title="Window-level detail was not exported for this candidate"' : '';
    return `<tr data-func="${r.func_name}" class="${cls}"${tip}>
      <td>${r.rank}</td><td>${r.score.toFixed(2)}</td><td>${r.best_stage}</td>
      <td class="${r.is_gt ? 'gt' : ''}">${r.func_name}</td><td>${r.label}</td><td>${r.tokens}</td>
  ...
  if (gtRows.length) {
    ...
    html += `<tr class="tableGroupSep"><td colspan="7">Full ranking (by rank)</td></tr>`;
  }
  html += rankings.map(rowHtml).join('');
  html += `</tbody></table>`;
  rankTableWrap.innerHTML = html;
  detailWrap.innerHTML = '<div class="empty">Click a candidate function in the table above to see the basic block / GT range / window score chart here.</div>';
